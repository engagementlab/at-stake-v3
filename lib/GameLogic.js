...
class GameLogic extends Common {
    ...
    */
    ...
    StartTimer(socket, timeAdded) {
        ...
        if(timeAdded) {
            let name = _.first(this.deliberateTimeQueue).username;
        ...
        this.eventEmitter.on('countdownEnded', (data, socket) => {
            ...
            if(data.countdownName === 'deliberateCountdown' && this.deliberateTimeQueue.length > 0) {
                // Remove first player in queue
                this.deliberateTimeQueue.shift();

                this.StartTimer(socket, true);
            }
    ...
    AddTime(socket, playerUid) {
        ...
        if(id === 'deliberate') {
            this.deliberateTimeQueue.push(player);
            
            if(this.deliberateTimeQueue.length > 1)
                return;
