    ...
    GetData(screenName) {
        ...
        switch(screenName) {
            ...
            case 'meet':
                ...
                let playerMap = _.mapObject(this.GetActivePlayers(),
                                (player) => { 
                                    return { 
            ...
            case 'doubledown':
                ...
                screenData = {
                    question: this._deck_data.questions[this._current_round],
                    players: this.Shuffler(_.mapObject(this.GetActivePlayers(), 
                                 (player) => { 
                                    return { 
                                        username: player.username,
                                        uid: player.uid, 
    ...
    ShowScreen(screenName, data, uniqueData, refreshCurrent, socket) {
        ...
        if(uniqueData) {
            ...
            this.Templates.Load('partials/shared/' + screenName, data, (html) => {
                ...
                let eventData = {name: screenName, html: html, phase: this.currentPhaseIndex, screen: this.currentScreenIndex};

                // Emit only to given socket, if specified
                if(socket !== undefined)
                    socket.emit(eventId, eventData);
                else
                    this.groupSocket.to(this.players_id).emit(eventId, eventData);
