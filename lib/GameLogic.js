...
class GameLogic extends Common {
    ...
    constructor() {
        ...
        // Player count who have read role card 
        this.playersRoleCardDone = -1,
    ...
    RoleWasRead() {
        ...
        this.playersRoleCardDone++;
        
        if(this.playersRoleCardDone === _.keys(this.GetActivePlayers()).length)
            this.groupSocket.to(this._current_decider.socket_id).emit('players:read_roles');
    ...
    GetData(screenName) {
        ...
        switch(screenName) {
            ...
            case 'doubledown':
                ...
                screenData = {
                    question: this._deck_data.questions[this._current_round],
                    players: _.mapObject(this.GetActivePlayers(), 
                                (player) => { 
                                    console.log(player)
                                return { 
                                    username: player.username,
    ...
    */
    ShowScreen(screenName, data, uniqueData, refreshCurrent, socket) {
        ...
        if(uniqueData) {
            ...
            function sendData(socket, phase, screen) {
                ...
                template.Load(path, outputData, (html) => {
                    ...
                    if(phase !== undefined && screen !== undefined)
                        socket.to(arrPlayerUnique[playerIndex].socket_id).emit(eventId, {html: html, phase: phase, screen: screen});
                    else
            ...
            _.each(this.GetActivePlayers(), function(player, id) {
            ...
            this.Templates.Load('partials/shared/' + screenName, data, (html) => {
                ...
                this.groupSocket.to(this._current_decider.socket_id).emit(eventId, {name: screenName, html: html, phase: this.currentPhaseIndex, screen: this.currentScreenIndex});
            ...
            this.Templates.Load('partials/shared/' + screenName, data, (html) => {
                ...
                if(socket !== undefined)
                    socket.emit(eventId, {name: screenName, html: html, phase: this.currentPhaseIndex, screen: this.currentScreenIndex});
                else
                    this.groupSocket.to(this.players_id).emit(eventId, {name: screenName, html: html});
