...
class GameLogic extends Common {
    ...
    constructor() {
        ...
        this.queueNextTimer = false;
    
        this.timerCallback = (data, socket) => {

            // Start timer again if somebody bought more deliberation time        
            if(data.countdownName === 'deliberateCountdown' && this._deliberate_time_queue.length > 0)
                this.queueNextTimer = true;
    ...
    StopCountdown() {
        ...
        // Initiate additional timer if one is in queue
    ...
    */
    ...
    StartTimer(socket, timeAdded) {
        ...
        this.eventEmitter.removeListener('countdownEnded', this.timerCallback);
        this.eventEmitter.on('countdownEnded', this.timerCallback);
    ...
    ProposalSelected(winnerUid) {
        ...
        this.Coins.GiveWinnerPot(winner);
