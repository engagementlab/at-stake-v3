...
class Common extends Core {
    ...
    Initialize(gameSession, callback) {
        ...
        super.Initialize(gameSession, require('keystone'), require('app-root-path'), () => {
            ...
            var Event = this.keystone.list('Event').model;
            ...
            var queryEvent = Event.find({}).exec();
    ...
    */
    PlayerReady(player, socket, assignDecider) {
        ...
        let playerRejoining = this.GetPlayerByUserId(player.uid) !== undefined;
        ...
        if(playerRejoining && wasDisconnected) {
            ...
            let role = this.GetPlayerById(player.socket_id).role;
            this.Templates.Load('partials/shared/rolecard', role, (roleHtml) => {
                ...
                this.eventEmitter.emit('playerReconnected', { is_decider: isDecider, socket: socket });
