...
class Common extends Core {
    ...
    Initialize(gameSession, callback) {
        ...
        super.Initialize(gameSession, () => {
            console.log("we have initialized the super")
        });
    ...
    PlayerReady(player, socket, isDecider) {
        ...
        if(isDecider) {
            ...
            // delete this._deck_data.roles[roleIndex];
        }
        ...
        if(this._current_players[player.uid] !== undefined)
        {
            ...
            socket.to(this.players_id).emit('players:update', this._current_players);
