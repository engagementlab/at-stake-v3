    ...
    */
    ...
    AssignRoleToPlayer(player, socket, isDecider) {
        ...
        if(!isDecider)
            ...
            this.Templates.Load('partials/shared/rolecard', playerObj.role, (html) => {

                // Tell new decider to start being decider
                if(this.groupSocket)
                    this.groupSocket.to(this._current_decider.socket_id).emit('game:decider', {facilitator: true, role: html});
